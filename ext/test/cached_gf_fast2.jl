println("------------------------------------")
println("|      Cached GF fast2 in CUDA     |")
println("------------------------------------")
			...
			for ordering in real_ac_grassmann_orderings
				for idx0 in 1;2
					...
					for band in 1:lattice.bands
						...
						g2 = cu_cached_greater_fast(idx0, lattice, A, cache=cache, band=band)
						@test norm(g1-g2)/norm(g1) < rtol
						g1 = [cached_lesser(lattice, idx0, i, A, cache=cache, band=band) for i in idx0:lattice.k]
						g2 = cu_cached_lesser_fast(idx0, lattice, A, cache=cache, band=band)
						@test norm(g1-g2)/norm(g1) < rtol
					...
					for band in 1:lattice.bands
						...
						g2 = cu_cached_greater_fast(idx0, lattice, A, B, cache=cache, band=band)
						@test norm(g1-g2)/norm(g1) < rtol
						g1 = [cached_lesser(lattice, idx0, i, A, B, cache=cache, band=band) for i in idx0:lattice.k]
						g2 = cu_cached_lesser_fast(idx0, lattice, A, B, cache=cache, band=band)
						@test norm(g1-g2)/norm(g1) < rtol	
					end
				end
			end
