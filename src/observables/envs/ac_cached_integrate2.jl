...
environments2(lattice::AbstractGrassmannLattice, A::Union{GrassmannMPS, Vector}, B::Vararg{GrassmannMPS}; 
						alg::IntegrationAlgorithm=ExactIntegrate()) = TwosideExpectationCache2(lattice, (A, B...))

struct TwosideExpectationCache2{M<:GrassmannMPS, G<:Tuple, L<:AbstractGrassmannLattice, Hl, Hr} <: AbstractExpectationCache
	A::M
	Bs::G
	lattice::L
	hleft::Hl
	hleft_scaling::Vector{Float64}
	hright::Hr	
	hright_scaling::Vector{Float64}
end
...
# cached_gf_fast2.jl
# idx0 = 1 will be the default for function in cached_gf_fast.jl
...
function cached_gf_fast(idx0::Int, lattice::AbstractGrassmannLattice, N::Int, A::GrassmannMPS, Bs::Vararg{GrassmannMPS}; 
			cache::TwosideExpectationCache2=environments2(lattice, A, Bs...),
...
function cached_gf_fast_normal_order(idx0::Int, lattice::AbstractGrassmannLattice, idx1::Int, A::GrassmannMPS, Bs::Vararg{GrassmannMPS};
	...
	for i in 3:N
		...
		left′ = left * GrassmannTransferMatrix(k, A2, cache.Bs...)
		scaling′ = scaling * (_normalize!(left′) / leftscaling(cache, k+1))
		GFt[i] = contract_center(left′, right) * scaling′ / contract_center(leftenv(cache, k+1), right)
	end
...
function cached_gf_fast_reverse_order(idx0::Int, lattice::AbstractGrassmannLattice, idx1::Int, A::GrassmannMPS, Bs::Vararg{GrassmannMPS};
                    				 cache::TwosideExpectationCache2=environments2(lattice, A, B...),
                    				 b1::Symbol, b2::Symbol, c1::Bool, c2::Bool, band::Int=1, kwargs...)
	@assert idx1 > idx0
	...
	for i in 3:N
		...
		right′ = GrassmannTransferMatrix(j, A2, cache.Bs...) * right 
		scaling′ = scaling * (_normalize!(right′) / rightscaling(cache, j-1))
		GFt[i] = contract_center(left, right′) * scaling′ / contract_center(left, rightenv(cache, j-1))
