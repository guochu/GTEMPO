...
get_Z(x::AbstractExpectationCache) = TK.scalar(x.hleft[end])
"""
...
cached_integrate_util(lattice::AbstractGrassmannLattice, j::Int, k::Int, cache::AbstractExpectationCache, A::Union{GrassmannMPS, Vector}, Bs::GrassmannMPS...; kwargs...) = _cached_integrate_util(
	lattice, j, k, cache, A, Bs...; kwargs...) / get_Z(cache)
...
# 	lattice, j, k, cache, A2, Bs...; kwargs...) / get_Z(cache)
...
get_Z(x::VectorExpectationCache) = sum(get_Z.(x.caches))
