...
function update_pair_left(left0::AbstractTensorMap{<:ElementarySpace, 1, 3}, j::Int, x::Vector, y::Vector, z::Vector; trunc=DefaultIntegrationTruncation) 
	...
	for (f1, f2) in fusiontrees(left)
		coef1 = (fermionparity(f2.uncoupled[2]) && fermionparity(f2.uncoupled[1])) ? -1 : 1
		coef2 = (fermionparity(f2.uncoupled[3]) && fermionparity(f2.uncoupled[1])) ? -1 : 1
		coef = coef1 * coef2
		if coef != 1
			left[f1, f2] .*= coef
		end
	...
	for (f1, f2) in fusiontrees(tmp1)
		coef1 = (fermionparity(f2.uncoupled[1]) && fermionparity(f1.uncoupled[2])) ? -1 : 1
		coef2 = (fermionparity(f2.uncoupled[1]) && fermionparity(f1.uncoupled[3])) ? -1 : 1
		coef3 = (fermionparity(f2.uncoupled[1]) && fermionparity(f1.uncoupled[4])) ? -1 : 1
		coef4 = (fermionparity(f1.uncoupled[4]) && fermionparity(f1.uncoupled[2])) ? -1 : 1
		coef5 = (fermionparity(f1.uncoupled[4]) && fermionparity(f1.uncoupled[3])) ? -1 : 1
		# println(coef1, " ", coef2, " ", coef3, " ", coef4, " ", coef5)
		coef = coef1 * coef2 * coef3 * coef4 * coef5
		if coef != 1
			tmp1[f1, f2] .*= coef
	...
	for (f1, f2) in fusiontrees(tmp1)
		...
		coef3 = (isodd(f2.uncoupled[1].n) && isodd(f1.uncoupled[5].n)) ? -1 : 1
		coef4 = (isodd(f1.uncoupled[5].n) && isodd(f1.uncoupled[3].n)) ? -1 : 1
	...
	for (f1, f2) in fusiontrees(threesitemps2)
		coef1 = (isodd(f1.uncoupled[6].n) && isodd(f1.uncoupled[5].n)) ? -1 : 1
		coef2 = (isodd(f1.uncoupled[7].n) && isodd(f1.uncoupled[5].n)) ? -1 : 1
		coef = coef1 * coef2 
		if coef != 1
			threesitemps2[f1, f2] .*= coef
		end
	...
	# trace physices
	left = TensorMap(ds->zeros(scalartype(left5), ds), space(left5, 1) ⊗ space(left5, 4) ⊗ space(left5, 5) ← space(left5, 6)' )
	for (f1, f2) in fusiontrees(left5)
		if f1.uncoupled[2] == f1.uncoupled[3]
			...
			@tensor left[f0, f2][1,3,4,5] += left5[f1, f2][1,2,2,3,4,5]
			# axpy!(1, tmp, r[f0, f2]) 
		end
	end	
...
function update_pair_right(right::AbstractTensorMap{<:ElementarySpace, 3, 1}, j::Int, x::Vector, y::Vector, z::Vector; trunc=DefaultIntegrationTruncation)
	...
	# trace physices
	right = TensorMap(ds->zeros(scalartype(right5), ds), space(right5, 1) ⊗ space(right5, 2) ← space(right5, 3)' ⊗ space(right5, 6)' )
	for (f1, f2) in fusiontrees(right5)
		if f2.uncoupled[2] == f2.uncoupled[3]
			f0 = FusionTree((f2.uncoupled[1],f2.uncoupled[4]), f2.coupled, (f2.isdual[1],f2.isdual[4]))
			@tensor right[f1, f0][1,2,3,5] += right5[f1, f2][1,2,3,4,4,5]
			# axpy!(1, tmp, r[f0, f2]) 
		end
	end	

	return permute(right, (1,2,3), (4,))
end
