...
function update_pair_right(right::AbstractTensorMap{<:ElementarySpace, 4, 1}, j::Int, x::Vector, y::Vector, z::Vector, u::Vector; trunc=DefaultIntegrationTruncation)
	posb = 2 * j
	@tensor tmp1[4 ;1 2 5 6 7] := u[posb][1,2,3] * right[3,4,5,6,7]
	for (f1, f2) in fusiontrees(tmp1)
	...
	@tensor tmp2[4;5 1 6 2 7 6] := z[posb][1,2,3] * tmp1[3,4,5,6,7,8]
	for (f1, f2) in fusiontrees(tmp2)
	...
	@tensor tmp2[1 ;4 5 6 2 7 8 9] := u[posb-1][1,2,3] * tmp3[3,4,5,6,7,8]
	for (f1, f2) in fusiontrees(tmp2)
		coef1 = (isodd(f2.uncoupled[4].n) && isodd(f2.uncoupled[1].n)) ? -1 : 1
		coef2 = (isodd(f2.uncoupled[4].n) && isodd(f2.uncoupled[2].n)) ? -1 : 1
		coef3 = (isodd(f2.uncoupled[4].n) && isodd(f2.uncoupled[3].n)) ? -1 : 1
	...
	return permute(right, (1,2,3,4), (7,))
