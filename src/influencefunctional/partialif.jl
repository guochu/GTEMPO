...
function hybriddynamicsstepper!(gmps::GrassmannMPS, lattice::RealGrassmannLattice1Order, corr::RealCorrelationFunction; band::Int=1, trunc::TruncationScheme=DefaultITruncation)
	...
	if lattice.k == 2
		pos1, pos2 = index(lattice, 1, conj=true, forward=true, band=band), index(lattice, 1, conj=false, forward=true, band=band)
		...
		apply!(exp(GTerm(pos1, pos2, coeff=η⁺⁻[1,1])), gmps)
		pos1, pos2 = index(lattice, 1, conj=true, forward=false, band=band), index(lattice, 1, conj=false, forward=true, band=band)
		apply!(exp(GTerm(pos1, pos2, coeff=η⁻⁺[1,1])), gmps)
		pos1, pos2 = index(lattice, 1, conj=true, forward=false, band=band), index(lattice, 1, conj=false, forward=false, band=band)
		apply!(exp(GTerm(pos1, pos2, coeff=η⁻⁻[1,1])), gmps)
		canonicalize!(gmps, alg=Orthogonalize(SVD(), trunc))
	end
	tmp1 = finalinfluencefunctional(lattice, η⁺⁺, trunc=trunc, fi=true, fj=true, band=band)
	tmp2 = finalinfluencefunctional(lattice, η⁺⁻, trunc=trunc, fi=true, fj=false, band=band)
	tmp3 = finalinfluencefunctional(lattice, η⁻⁺, trunc=trunc, fi=false, fj=true, band=band)
	tmp4 = finalinfluencefunctional(lattice, η⁻⁻, trunc=trunc, fi=false, fj=false, band=band)
...
function hybriddynamicsstepper!(gmps::GrassmannMPS, lattice::RealGrassmannLattice2Order, corr::RealCorrelationFunction; 
	...
	tmp1 = finalinfluencefunctional(lattice, η⁺⁺, finalize=finalize, trunc=trunc, fi=true, fj=true, band=band)
	tmp2 = finalinfluencefunctional(lattice, η⁺⁻, finalize=finalize, trunc=trunc, fi=true, fj=false, band=band)
	tmp3 = finalinfluencefunctional(lattice, η⁻⁺, finalize=finalize, trunc=trunc, fi=false, fj=true, band=band)
	tmp4 = finalinfluencefunctional(lattice, η⁻⁻, finalize=finalize, trunc=trunc, fi=false, fj=false, band=band)
