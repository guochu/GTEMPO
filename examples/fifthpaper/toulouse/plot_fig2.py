...
ts_real_analytic, gf_real_analytic = read_real_analytic(t, mu, dt=0.05)
# ax[0,0].plot(ts_real_analytic, gf_real_analytic, ls='-', color='k', linewidth=linewidth, label=r'Analytic')
# ax[1,0].plot(ts_real_analytic, gf_real_analytic, ls='-', color='k', linewidth=linewidth, label=r'Analytic')
print(len(ts_real_analytic))

ts_imag_analytic, gf_imag_analytic = read_imag_analytic(beta, mu, dt=0.1)
# ax[0,1].plot(ts_imag_analytic, gf_imag_analytic, ls='-', color='k', linewidth=linewidth, label=r'Analytic')
# ax[1,1].plot(ts_imag_analytic, gf_imag_analytic, ls='-', color='k', linewidth=linewidth, label=r'Analytic')
...
for (i, dt) in enumerate(all_dt):
	taus, gtau, ts_tempo, gt_tempo, lt_tempo, gf_tempo, bds = read_mixed_tempo(beta, dtau, t, mu, dt, chi=chi)

	ratio = round(dt / 0.05)

	gf_real_analytic_match = gf_real_analytic[0:len(gf_real_analytic):ratio]

	err_real = gf_real_analytic_match - gf_tempo

	ax[0,0].plot(ts_tempo, err_real, ls='--', color=colors[i], linewidth=linewidth, label=r'mixed, $\delta t=%s$'%(dt))


	err_imag = gf_imag_analytic - gtau

	ax[0,1].plot(taus, err_imag, ls='--', color=colors[i], linewidth=linewidth, label=r'mixed, $\delta t=%s$'%(dt))
...
ax[0,1].set_ylabel(r'$G_{Ana}(\tau)-G(\tau)$', fontsize=fontsize)
...
for (i, dtau) in enumerate(all_dtau):
	...
	ax[1,0].plot(ts_tempo, err_real, ls='--', color=colors[i], linewidth=linewidth, label=r'mixed, $\delta \tau=%s$'%(dtau))

	if dtau < 0.1:
		gtau = gtau[0:len(gtau):2]
		taus = ts_imag_analytic
	else:
		ratio = round(dtau / 0.1)
		gf_imag_analytic = gf_imag_analytic[0:len(gf_imag_analytic):ratio]

	err_imag = gf_imag_analytic - gtau

	ax[1,1].plot(taus, err_imag, ls='--', color=colors[i], linewidth=linewidth, label=r'mixed, $\delta \tau=%s$'%(dtau))


ax[1,0].set_ylabel(r'$-{\rm Im}[G_{Ana}(t) - G(t)]$', fontsize=fontsize)
...
ax[1,1].set_ylabel(r'$G_{Ana}(\tau)-G(\tau)$', fontsize=fontsize)
