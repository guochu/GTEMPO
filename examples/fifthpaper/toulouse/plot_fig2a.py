...
for (i, dt) in enumerate(all_dt):
	...
	ax[0,0].plot(ts_tempo, err_real, ls='--', color=colors[i], linewidth=linewidth, label=r'$\delta t=%s$'%(dt))


	err_imag = gf_imag_analytic - gtau
	errs_imag.append(mse_error(gf_imag_analytic, gtau))

	ax[1,0].plot(taus, err_imag, ls='--', color=colors[i], linewidth=linewidth, label=r'$\delta t=%s$'%(dt))
