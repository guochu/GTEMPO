...
spectrum_func(;α=1, ω=1) = DiracDelta(ω=ω, α=α)
...
function main_real_analytic(ϵ_d; β=1, t=1, N=100, ω=1, α=1)
	# ϵ_d = 0.5
	δt=t/N
	ts = collect(0:δt:t)
	g1 = [independentbosons_greater(spectrum_func(ω=ω, α=α), tj, β=β, ϵ_d=-ϵ_d) for tj in ts]
	g2 = [independentbosons_lesser(spectrum_func(ω=ω, α=α), tj, β=β, ϵ_d=-ϵ_d) for tj in ts]

	data_path = "result/noninteracting_sm_analytic_real_beta$(β)_mu$(ϵ_d)_t$(t)_N$(N)_omega$(ω)_alpha$(α).json"
...
function main_mixed(ϵ_d; β=1, Nτ=20, t=1, Nt=100, ω=1, chi = 100, α=1)
	# ϵ_d = 0.5
	δτ = β / Nτ
	δt = t / Nt

	println("Nt=", Nt, " δt=", δt, " Nτ=", Nτ, " δτ=", δτ, " ϵ_d=", ϵ_d, " β=", β, " chi=", chi, " ω=", ω, " α=", α)
	...
	mpspath = "data/noninteracting_sm_mixedgtempo_beta$(β)_dtau$(δτ)_t$(t)_dt$(δt)_omega$(ω)_alpha$(α)_chi$(chi).mps"
	if ispath(mpspath)
		println("load MPS-IF from path ", mpspath)
		mpsI = Serialization.deserialize(mpspath)
	else
		println("computing MPS-IF...")
		bath = bosonicbath(spectrum_func(ω=ω, α=α), β=β)
	...
	data_path = "result/noninteracting_sm_mixedgtempo_beta$(β)_dtau$(δτ)_t$(t)_dt$(δt)_mu$(ϵ_d)_omega$(ω)_alpha$(α)_chi$(chi).json"
