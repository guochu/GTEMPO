...
def read_real_tempo_int(beta, t, N, U, J, mu, omega0, alpha0, omega1, alpha1, chi=80):
	dt = t / N
	filename = 'result/interacting_realgtempo_beta%s_t%s_dt%s_omega0%s_alpha0%s_omega1%s_alpha1%s_U%s_J%s_mu%s_chi%s.json'%(beta, t, dt, omega0, alpha0, omega1, alpha1, U, J, mu, chi)
...
def read_neq_ed_int(beta, t, N, U, J, mu, omega0, alpha0, omega1, alpha1):
	filename = 'result/interacting_neq_ED_real_beta%s_U%s_J%s_mu%s_t%s_N%s_omega0%s_alpha0%s_omega1%s_alpha1%s.json'%(beta, U, J, mu, t, N, omega0, alpha0, omega1, alpha1)
	with open(filename, 'r') as f:
		data = f.read()
		data = json.loads(data)
	gt = parse_complex_array(data['gt'])
...
# real time data
ts, gt, lt, nn = read_neq_ed_int(beta, t, Nt, U, J, mu, omega0, alpha0, omega1, alpha1)
...
ax[2,0].annotate(r'(e)', xy=annotate_xy,xycoords='axes fraction', fontsize=fontsize)
...
ax[2,1].annotate(r'(f)', xy=annotate_xy,xycoords='axes fraction', fontsize=fontsize)
