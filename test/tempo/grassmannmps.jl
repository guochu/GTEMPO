...
@testset "GrassmannMPS: multiplications" begin
	...
	alg5 = DMRG2(trunc, initguess=:svd)
	alg6 = DMRG2(trunc, initguess=:rand, maxiter=10)
	alg7 = DMRG2(trunc, initguess=:pre, maxiter=10)
...
@testset "GrassmannMPS: integration 4, 5" begin
	rtol = 1.0e-4
	...
	for N in (1,2)
		for bands in (1,2)
			for ordering in real_grassmann_orderings
				...
				Z1 = integrate(lattice, AB, CDE)
				Z2 = integrate(lattice, A, B, C, D, E)
